...
//        TextTabFragment.thirdFragment;
    ...
    @Override
    public void FirstAutoArrange(List<realCourseData> realList,List<Boolean>Neccesary) {
        //
        realCourseData[] temp = new realCourseData[realList.size()];
        for(int i = 0 ;i<realList.size();i++){
            realCourseData tempr = realList.get(i);
            if(Neccesary.get(i) == true) {
                tempr.setNecessary(1);
            }else{
                tempr.setNecessary(0);
            }
            realList.set(i,tempr);
        }
